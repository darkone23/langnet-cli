"""
Structured logging configuration for langnet.
...
def _filter_by_level(logger, method_name, event_dict):
    ...
    effective_level = _log_state.level or logging.WARNING
...
def setup_logging(level: int | str | None = None, force: bool = False) -> None:
    """Configure structured logging for langnet.* loggers."""
    if _log_state.configured and not force:
        return

    resolved_level = _coerce_level(level)
    _log_state.level = resolved_level
...
def scoped_context(**kwargs: object) -> Iterator[None]:
