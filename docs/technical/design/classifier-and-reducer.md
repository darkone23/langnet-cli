...
**Prerequisites**: `witness-contracts.md`, `entry-parsing.md`, `tool-fact-architecture.md`
...
**Architecture Note**: The semantic reducer consumes Claims from the universal layer (see `tool-fact-architecture.md`). Tool-specific facts are transformed to Claims before reaching this pipeline:
...
1. **Entry Parsing** (see `entry-parsing.md`): Parse raw dictionary entries into `ParsedEntry` objects
...
**What is NOT a WSU type**: Grammatical metadata (POS, gender, conjugation) stays at the entry level. WSU types are for semantic content that requires similarity analysis. See `entry-parsing.md` for the full design principle.
...
* [ ] **Entry parsing implemented** (prerequisite - see `entry-parsing.md`)
