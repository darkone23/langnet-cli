...
- CDSL projection: **current** handler queries DuckDB read-only (mw + ap90) with accent/IAST→SLP1 matching and emits `has_sense` glosses with `source_ref` (`mw:<lnum>`). TODO: parse `sense_lines`/domains/register/root/grammar_refs` via `codesketch/cologne/parser.py` and emit structured triples; add fixtures.
...
- Sktuser guess path: strip any remaining SLP1 echoes; optionally propagate dictionary URLs when only one anchor exists; add fixtures for guess fallback (e.g., viṣṇu acc/nom) to guard parsing and claim output.
- Verification target: `just triples-dump san zmazruvardhana heritage` and `yogānuśāsanam` should emit correctly grouped segments and parsed features with color hints; rerun `mahashmashana` to confirm compound parsing survives; include a guess fallback case in fixtures.
