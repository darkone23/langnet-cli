...
import type { ReaderIndexView } from './index-storage';
...
export type ReaderRouteUrlUpdateInput = {
	currentUrl: string;
	state: Partial<ReaderRouteState>;
...
export function applyReaderRouteOverrides(
	state: Partial<ReaderRouteState>,
	overrides: ReaderRouteOverrides = {}
) {
	const next = { ...state };
	for (const [key, value] of Object.entries(overrides) as [
		...
		else if (value !== undefined) next[key] = value as never;
	}
	return next;
}

export function buildReaderRouteUrlUpdate({
	currentUrl,
	state,
	overrides = {}
}: ReaderRouteUrlUpdateInput) {
	const nextState = applyReaderRouteOverrides(state, overrides);
	const nextUrl = `/reader${buildReaderRouteSearch(nextState)}`;
