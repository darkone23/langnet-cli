<script lang="ts">
	import type { ReaderSearchQueryCandidate, ReaderSearchResult } from '$lib/reader';
	...
	let { results, queryCandidates, candidateLabel, onOpenResult, onOpenAuthor }: Props = $props();
</script>

{#if queryCandidates.length}
	<div class="orion-reader-work-meta">
		{#each queryCandidates as candidate}
			<small title={candidate.explanation || candidate.kind}>
				{candidate.kind.replace(/_/g, ' ')} · {candidateLabel(candidate)}
			</small>
		{/each}
	</div>
{/if}
	...
	{#each results as result}
		<article class="orion-reader-work-row">
			<div class="orion-reader-work-row-main">
				<button type="button" class="orion-reader-work-open" onclick={() => onOpenResult(result)}>
					<strong>
