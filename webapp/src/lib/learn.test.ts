...
assert.equal(href, '/q?lang=san&q=gam&backend=cli&translation=cache&dictionary=all&load=yes');
assert.equal(
	practiceHref({ language: 'grc', word: 'logon', gloss: 'word', note: 'probe' }),
	'/q?lang=grc&q=logon&backend=cli&translation=cache&dictionary=all&load=yes'
);
assert.equal(
	practiceHref({ language: 'lat', word: 'lupum', gloss: 'wolf', note: 'probe' }),
	'/q?lang=lat&q=lupum&backend=cli&translation=cache&dictionary=all&load=yes'
);
